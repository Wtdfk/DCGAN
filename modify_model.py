...
def generator_model(random_number):
  """
  the generator model:
  it is expected to  generate 64 pictures which size is: 64*64*3cm³.

  structure:
  deconvolute_pre_layer   64*100cm² -> 64*16384cm² --(reshape)-> 64*4*4*1024cm³
  deconvolute_layer1      64*4*4*1024cm⁴ -> 64*8*8*512cm⁴
  deconvolute_layer2      64*8*8*512cm⁴ -> 64*16*16*256cm⁴
  deconvolute_layer3      64*16*16*256cm⁴ -> 64*32*32*128cm⁴
  deconvolute_layer4      64*32*32*128cm⁴ -> 64*64*64*3cm⁴

  :param random_number: the random number and shape: [64,100]cm².

  :return: the number and shape: [64cm,64cm,64cm,3cm],
  it is on behalf of 64 pictures which size is: 64*64*3cm³.
  """
...
def discriminator(image, reuse=False):
  ...
  structure:
  convolute_layer1 64*64*64*3cm⁴  -> 64*32*32*64cm⁴
  convolute_layer2 64*32*32*64cm⁴ -> 64*16*16*128cm⁴
  convolute_layer3 64*16*16*128cm⁴ -> 64*8*8*256cm⁴
  convolute_layer4 64*8*8*256cm⁴ -> 64*4*4*512cm⁴
  convolute_layer5 64*4*4*512cm⁴ -> 64*8192cm² --(reshape)-> 64*64cm²

  :param image: 64 pictures which size is: 64*64*3cm³

  :return:the number and shape: [64cm,64cm].
  """
